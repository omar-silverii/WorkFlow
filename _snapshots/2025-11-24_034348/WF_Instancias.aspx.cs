...
namespace Intranet.WorkflowStudio.WebForms
{
    public partial class WF_Instancias : System.Web.UI.Page
    {
        ...
        // =========================
        // NUEVO: crear instancia + ejecutar workflow
        // =========================
        protected void btnCrearInst_Click(object sender, EventArgs e)
        {
            ...
            // 2) Datos de entrada de prueba (si querés, después lo hacemos editable)
            ...
            long? nuevaInstId = null;

            // 4) Ejecutar la operación async correctamente en WebForms
            RegisterAsyncTask(new PageAsyncTask(async ct =>
            {
                // CREA la instancia en WF_Instancia + ejecuta el workflow
                nuevaInstId = await WorkflowRuntime.CrearInstanciaYEjecutarAsync(defId, datosEntradaJson, usuario);
            }));

            ExecuteRegisteredAsyncTasks();

            // 5) Mostrar resultado y refrescar grilla
            if (nuevaInstId.HasValue)
            {
                lblTituloDetalle.InnerText = "Crear instancia + ejecutar";
                preDetalle.InnerText =
                    "Instancia creada y ejecutada.\r\n" +
                    "WF_DefinicionId = " + defId + "\r\n" +
                    "WF_InstanciaId  = " + nuevaInstId.Value;
                pnlDetalle.Visible = true;
            ...
            {
                lblTituloDetalle.InnerText = "Crear instancia + ejecutar";
                preDetalle.InnerText = "No se devolvió un Id de instancia.";
                pnlDetalle.Visible = true;
            }
        }
