...
(function (global) {
    ...
    // ====== Grupos (lo que ves en la toolbox y en qué orden aparece)
    var GROUPS = [
        {
            name: "Disparadores", items: [
                "trigger.webhook", "trigger.cron", "trigger.queue"
            ]
        },
        {
            name: "Control", items: [
                "control.if", "control.switch", "control.parallel", "control.join", "control.loop", "control.delay", "control.retry", "control.ratelimit"
            ]
        },
        {
            name: "Datos e Integraciones", items: [
                "http.request", "data.sql", "data.redis.get", "data.redis.set", "file.read", "file.write", "email.send", "chat.notify", "cloud.storage", "queue.publish", "queue.consume", "ftp.get", "ftp.put", "doc.entrada"
            ]
        },
        {
            name: "Transformación y Lógica", items: [
                "transform.map", "code.function", "code.script", "state.vars", "config.secrets", "ai.call"
            ]
        },
        {
            name: "Utilidad / Operación", items: [
                "util.start", "util.end", "util.logger", "util.notify", "util.error", "util.subflow"
            ]
        }
    ];
    ...
    var PARAM_TEMPLATES = {
        ...
        "doc.entrada": {
            modo: "simulado",              // simulado | upload | carpeta | api
            extensiones: ["pdf", "docx"],
            maxMB: 10,
            salida: "documento"            // nombre del objeto en contexto
        },
