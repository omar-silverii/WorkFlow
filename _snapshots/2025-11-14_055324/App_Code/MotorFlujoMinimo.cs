    ...
    public class ManejadorSql : IManejadorNodo
    {
        ...
        public async Task<ResultadoEjecucion> EjecutarAsync(ContextoEjecucion ctx, NodeDef nodo, CancellationToken ct)
        {
            ...
            if (p.TryGetValue("parameters", out var parObj))
            {
                ...
                else if (parObj is Dictionary<string, object> dict)
                    sqlParams = new Dictionary<string, object>(dict, StringComparer.OrdinalIgnoreCase);
            }
            ...
            using (var cmd = new SqlCommand(sql, cn))
            {
                if (sqlParams != null)
                {
                    ...
                    {
                        var expanded = ExpandTemplateValue(kv.Value, ctx); // <<< NUEVO
                        cmd.Parameters.AddWithValue("@" + kv.Key, expanded ?? DBNull.Value);
                    }
                }
        ...
        private static readonly System.Text.RegularExpressions.Regex RxWhole =
        new System.Text.RegularExpressions.Regex(@"^\s*\$\{(?<path>[^}]+)\}\s*$", System.Text.RegularExpressions.RegexOptions.Compiled);

        private static readonly System.Text.RegularExpressions.Regex RxAny =
            new System.Text.RegularExpressions.Regex(@"\$\{(?<path>[^}]+)\}", System.Text.RegularExpressions.RegexOptions.Compiled);

        // Si el valor es string y es solo ${...} => devuelve el valor tipado;
        // si contiene varios ${...} mezclados con texto => devuelve string con reemplazos;
        // si no es string => lo deja igual.
        private static object ExpandTemplateValue(object value, ContextoEjecucion ctx)
        {
            if (value == null) return null;

            if (value is string s)
            {
                var m = RxWhole.Match(s);
                if (m.Success)
                {
                    var path = m.Groups["path"].Value.Trim();
                    var resolved = ContextoEjecucion.ResolverPath(ctx.Estado, path);
                    return resolved; // tipado
                }

                // Reemplazo parcial a string
                var str = RxAny.Replace(s, match =>
                {
