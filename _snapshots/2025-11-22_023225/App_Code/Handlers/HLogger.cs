...
namespace Intranet.WorkflowStudio.WebForms
{
    ...
    public class HLogger : IManejadorNodo
    {
        ...
        public Task<ResultadoEjecucion> EjecutarAsync(ContextoEjecucion ctx, NodeDef nodo, CancellationToken ct)
        {
            ...
            var level = GetString(p, "level") ?? "Info";
            var message = GetString(p, "message") ?? string.Empty;

            // Interpolar usando ctx.Estado (soporta JObject/JArray/JValue, IDictionary, POCOs, índices [0])
            level = Interpolate(level, ctx.Estado);
            message = Interpolate(message, ctx.Estado);

            ctx.Log($"[Logger] [{level}] {message}");
