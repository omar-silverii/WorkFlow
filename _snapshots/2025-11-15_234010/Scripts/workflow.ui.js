...
(function () {
    ...
    // Fallbacks por si no vinieron plantillas desde el server
    PARAM_TEMPLATES['data.sql'] = PARAM_TEMPLATES['data.sql'] || {
        connectionStringName: 'DefaultConnection',
        query: 'SELECT TOP 10 * FROM dbo.Usuarios ORDER BY Id DESC',
        parameters: {}
    };

    PARAM_TEMPLATES['http.request'] = PARAM_TEMPLATES['http.request'] || {
        url: '/Api/Ping.ashx',
        method: 'GET',
        headers: {},
        query: {},
    ...
    function renderInspector() {
        ...
        function kvTable(obj) {
            ...
            var tbl = createEl('table', 'kv');
            wrap.appendChild(tbl);

            function addRow(k, v) {
                var tr = createEl('tr');

                var kIn = createEl('input', 'input');
                kIn.placeholder = 'nombre';
                if (k != null) kIn.value = k;

                var vIn = createEl('input', 'input');
                ...
                // Cuando escribís en la última fila, agrega otra vacía
                function maybeAddAnother() {
                    var rows = tbl.querySelectorAll('tr');
                    var isLast = (rows.length && rows[rows.length - 1] === tr);
                    if (isLast && (kIn.value.trim() !== '' || vIn.value.trim() !== '')) {
                        addRow('', '');
                    }
                }
            ...
            // Una fila vacía de arranque
            addRow('', '');

            // Devolver como objeto {k: v, ...}
        ...
        if (selected.type === 'node') {
            ...
            if (n.key === 'data.sql') {
                ...
                // === Eventos (con preventDefault/stopPropagation por si acaso) ===
                bTplSql.onclick = function (ev) {
                    ev.preventDefault(); ev.stopPropagation();
                    var tpl = (window.PARAM_TEMPLATES && window.PARAM_TEMPLATES['data.sql']) || {};
                    if (!inpConn.value) inpConn.value = tpl.connection || '';
                    if (!inpCsn.value) inpCsn.value = tpl.connectionStringName || '';
                    if (!taQry.value) taQry.value = tpl.query || tpl.commandText || '';

                    // recargar los parámetros de la plantilla
                    sParams.innerHTML = '<div class="label">Parameters (k → v)</div>';
                    kvP = kvTable(tpl.parameters || {});
                    sParams.appendChild(kvP);
                };

                bSaveSql.onclick = function (ev) {
                    ev.preventDefault(); ev.stopPropagation();
                    var next = Object.assign({}, n.params || {});
                    delete next.connection; delete next.connectionStringName; delete next.query; delete next.commandText;
                    ...
                    if (q) next.query = q; // usamos "query"
