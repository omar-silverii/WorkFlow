...
namespace Intranet.WorkflowStudio.WebForms
{
    public partial class WorkflowUI : System.Web.UI.Page
    {
        ...
        private void GuardarDefinicionEnSql(string json)
        {
            ...
            // Detectar meta de plantilla (si vino) para prefijos
            ...
            // NUEVO: si el usuario tipeó un nombre, lo uso
            string nombrePost = Request.Form["txtNombreWf"];
            string nombre = !string.IsNullOrWhiteSpace(nombrePost)
                ? nombrePost.Trim()
                : (esEmision ? "Emisión de Póliza " : "Workflow desde UI ") + DateTime.Now.ToString("dd/MM/yyyy HH:mm");

            // Código: slug del nombre + timestamp (si hay nombre), o el viejo prefijo
            string codigoBase = !string.IsNullOrWhiteSpace(nombrePost)
                ? Slug(nombrePost)
                : (esEmision ? "EMISION" : "WF");

            string codigo = $"{codigoBase}-{DateTime.Now:yyyyMMdd-HHmmss}";
