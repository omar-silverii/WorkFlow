...
namespace Intranet.WorkflowStudio.WebForms
{
    ...
    public class ManejadorSql : IManejadorNodo
    {
        ...
        public async Task<ResultadoEjecucion> EjecutarAsync(ContextoEjecucion ctx, NodeDef nodo, CancellationToken ct)
        {
            ...
            object cnnNameObj, cnnDirectObj;
            ...
            {
                cnnString = System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString;
            }

            // 2) tomar el SQL (query/commandText) y expandir placeholders ${...}
            string sql = null;
            object cmdObj, qObj;
            ...
            sql = Expand(sql, ctx.Estado); // <-- templating en el texto SQL

            // 3) parámetros opcionales (k → v) con templating también en el valor
            Dictionary<string, object> sqlParams = null;
            object parObj;
            if (p.TryGetValue("parameters", out parObj) && parObj is Newtonsoft.Json.Linq.JObject jobj)
            {
                sqlParams = jobj.ToObject<Dictionary<string, object>>();
            }
            if (sqlParams == null) sqlParams = new Dictionary<string, object>();

            // expandir cada valor si es string
            var keys = new List<string>(sqlParams.Keys);
            foreach (var key in keys)
            {
                var val = sqlParams[key];
                var s = val as string;
                if (!string.IsNullOrEmpty(s))
                {
                    sqlParams[key] = Expand(s, ctx.Estado);
                }
            }

            // 4) ejecutar
            using (var cn = new System.Data.SqlClient.SqlConnection(cnnString))
