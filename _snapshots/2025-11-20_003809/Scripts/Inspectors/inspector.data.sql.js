...
(function () {
    ...
    window.WF_Inspector.register('data.sql', function (selected, state, ui, helpers) {
        ...
        var idA = 'r_csn_' + n.id, idB = 'r_conn_' + n.id;

        wrapConnMode.innerHTML =
            '<label style="margin-right:10px;"><input type="radio" name="connMode_' + n.id + '" id="' + idA + '"> connectionStringName</label>' +
            '<label><input type="radio" name="connMode_' + n.id + '" id="' + idB + '"> connection</label>';
        ...
        function refreshConnMode() {
            var useCSN = document.getElementById(idA).checked;
