        ...
        if (selected.type === 'node') {
            ...
            if (n.key === 'data.sql') {
                ...
                bTplSql.onclick = function () {
                    ...
                    // Sugerencias por defecto (no pisan lo que ya tengas cargado)
                    if (!inpCsn.value) inpCsn.value = tpl.connectionStringName || 'DefaultConnection';
                    if (!inpConn.value) inpConn.value = tpl.connection || '';
                    if (!taQry.value) taQry.value = tpl.query || 'SELECT 1 AS Uno;';
                    // parameters de plantilla
                ...
                bSaveSql.onclick = function () {
                    var next = Object.assign({}, n.params || {});
                    // limpiar campos previos para evitar duplicados
                    ...
                    n.params = next;
                    ensurePosition(n);
                    var elN = nodeEl(n.id); if (elN) elN.querySelector('.node__title').textContent = n.label;
                    renderInspector();
                };
                ...
                bDelSql.onclick = function () {
                    edges = edges.filter(function (e) { return e.from !== n.id && e.to !== n.id; });
                    nodes = nodes.filter(function (x) { return x.id !== n.id; });
                    var elN = nodeEl(n.id); if (elN) elN.remove();
                    drawEdges(); select(null);
                };
