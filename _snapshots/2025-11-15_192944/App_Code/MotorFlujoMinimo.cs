...
namespace Intranet.WorkflowStudio.WebForms
{
    ...
    public class ManejadorSql : IManejadorNodo
    {
        ...
        // Expande ${ruta} contra ctx.Estado. Si la cadena es SOLO una expr, devuelve el tipo original.
        private static object Expand(object value, ContextoEjecucion ctx)
        {
            ...
            if (value is string s)
            {
                ...
                {
                    // ¿solo una expresión?
                    var mSolo = System.Text.RegularExpressions.Regex.Match(s, @"^\s*\$\{([^}]+)\}\s*$");
                    if (mSolo.Success)
                    {
                        var path = mSolo.Groups[1].Value.Trim();
        ...
        public async Task<ResultadoEjecucion> EjecutarAsync(ContextoEjecucion ctx, NodeDef nodo, CancellationToken ct)
        {
            ...
            // 1) Cadena de conexión: connectionStringName > connection > DefaultConnection
            string cnnString = null;
            if (p.TryGetValue("connectionStringName", out var cnnNameObj) && !string.IsNullOrWhiteSpace(Convert.ToString(cnnNameObj)))
                cnnString = ConfigurationManager.ConnectionStrings[Convert.ToString(cnnNameObj)].ConnectionString;
            else if (p.TryGetValue("connection", out var cnnDirectObj) && !string.IsNullOrWhiteSpace(Convert.ToString(cnnDirectObj)))
            ...
            // 3) Parámetros (del inspector K→V)
            Dictionary<string, object> sqlParams = null;
            if (p.TryGetValue("parameters", out var parObj))
            {
                if (parObj is Newtonsoft.Json.Linq.JObject jobj) sqlParams = jobj.ToObject<Dictionary<string, object>>();
                else if (parObj is Dictionary<string, object> dict) sqlParams = new Dictionary<string, object>(dict);
            }
            if (sqlParams == null) sqlParams = new Dictionary<string, object>();
            ...
            using (var cmd = new SqlCommand(sql, cn))
            {
                ...
                {
                    var val = Expand(kv.Value, ctx);
                    cmd.Parameters.AddWithValue("@" + kv.Key, val ?? DBNull.Value);
                }

                await cn.OpenAsync(ct);

                // 4) SELECT (o WITH) => devolvemos filas y resultado; DML => filas afectadas
                var isSelect = System.Text.RegularExpressions.Regex.IsMatch(sql, @"^\s*(SELECT|WITH)\b", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                ...
                if (isSelect)
                {
                    using (var rdr = await cmd.ExecuteReaderAsync(ct))
                    {
                        ...
                        ctx.Estado["sql.rows"] = rows.Count;
                        ctx.Estado["sql.result"] = rows;         // para usar después
                        ctx.Log("[SQL] SELECT filas=" + rows.Count);
                    }
                }
                else
                {
                    var affected = await cmd.ExecuteNonQueryAsync(ct);
                    ctx.Estado["sql.rows"] = affected;
                    ctx.Log("[SQL] DML filas=" + affected);
                }
            }
