...
namespace Intranet.WorkflowStudio.WebForms
{
    public partial class WF_Instancias : System.Web.UI.Page
    {
        ...
        protected async void gvInst_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "VerLog")
            {
                VerLog(instId);
            }
            else if (e.CommandName == "Reejecutar")
            {
                string usuario = (User?.Identity?.IsAuthenticated ?? false) ? User.Identity.Name : "wf.ui";
                var nueva = await Intranet.WorkflowStudio.Runtime.WorkflowRuntime.ReejecutarInstanciaAsync(instId, usuario);

                // feedback simple
                lblTituloDetalle.InnerText = "Nueva instancia creada: " + nueva;
                preDetalle.InnerText = "Se re-ejecutó la instancia " + instId + " → nueva Id = " + nueva;
