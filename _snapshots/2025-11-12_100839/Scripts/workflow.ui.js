        ...
        if (selected.type === 'node') {
            ...
            if (n.key === 'http.request') {
                ...
                bTpl.onclick = function () {
                    var tpl = (window.PARAM_TEMPLATES && window.PARAM_TEMPLATES['http.request']) || {};
                    inpUrl.value = inpUrl.value || (tpl.url || '/Api/Ping.ashx');
                    sel.value = ((p.method || tpl.method || 'GET') + '').toUpperCase();
                    if (!inpCT.value) inpCT.value = tpl.contentType || 'application/json';
                    if (!taBody.value) taBody.value = (tpl.body == null ? '' : (typeof tpl.body === 'string' ? tpl.body : JSON.stringify(tpl.body, null, 2)));
                };
            ...
            if (n.key === 'control.if') {
                ...
                bTplIf.onclick = function () {
                    var tpl = (window.PARAM_TEMPLATES && window.PARAM_TEMPLATES['control.if']) || {};
                    inpExpr.value = inpExpr.value || tpl.expression || '${payload.status} == 200';
                };
            ...
            if (n.key === 'data.sql') {
                ...
                bTplSql.onclick = function () {
                    ...
                    // parameters al panel
                    var obj = tpl.parameters || {};
                    // re-render simple: sobreescribir el área
