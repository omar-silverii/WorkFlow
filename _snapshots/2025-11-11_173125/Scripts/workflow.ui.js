...
(function () {
    ...
    function renderInspector() {
        ...
        if (selected.type === 'node') {
            ...
            bSave.onclick = function () { e.condition = inpCond.value || 'always'; drawEdges(); renderInspector(); };
    ...
    function buildWorkflow() {
        ...
        nodes.forEach(function (n) { wf.Nodes[n.id] = { Id: n.id, Type: n.key, Parameters: n.params || {} } });
        edges.forEach(function (e) { wf.Edges.push({ From: e.from, To: e.to, Condition: e.condition || 'always' }) });
        return wf;
    }
    ...
    function init() {
        ...
        // Restaurar si el server mandó algo
            ...
            try {
                ...
                if (wf && wf.Nodes) {
                    ...
                    Object.keys(wf.Nodes).forEach(function (id) {
                        var n = wf.Nodes[id];
                        // posición no viene en JSON del motor, así que las tiramos centradas
                        var meta = findCat(n.Type) || { key: n.Type, label: n.Type, tint: '#94a3b8' };
                        var x = 120 + (Object.keys(nodes).length * 40);
                        var y = 120 + (Object.keys(nodes).length * 10);
                        var newN = {
                            id: id,
                            key: meta.key,
                            label: meta.label,
        ...
        if ($('btnDemo')) $('btnDemo').addEventListener('click', function () {
            ...
            if (logNodeObj) { logNodeObj.params = { level: 'Error', message: 'Falló HTTP' }; }
